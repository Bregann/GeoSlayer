    ...
    IUserContextHelper userContextHelper) : ControllerBase
{
    ...
    [HttpPost("queue/{key}")]
    public async Task<ActionResult<CraftDto>> QueueCraft(string key, CancellationToken ct) =>
        Ok(await crafting.QueueCraft(await CurrentPlayerId(ct), key, ct));

    [HttpDelete("queue/{id:int}")]
    public async Task<IActionResult> CancelCraft(int id, CancellationToken ct)
    {
    ...
    public async Task<ActionResult<List<PlayerItemDto>>> SetEquipped(
        int id, [FromBody] EquipRequest request, CancellationToken ct) =>
